...
- **Contents:** `/usr/bin/lazysubmodules`, the short name `/usr/bin/lsm`,
  ...
  the sources.
- **Version:** derived from the Git history at build time, so it
  ...
  release of the same version. `lazysubmodules version` prints it, with
  the commit and the commit date.
...
| State | Meaning |
|---|---|
| `ok` | HEAD equals the lock commit, and no other target is available locally |
| `behind` | `update` would select another mode, ref or commit than the lock entry records, or there is no lock entry yet |
...
| `missing-ref` | The configured ref is invalid or not found in the local refs |
